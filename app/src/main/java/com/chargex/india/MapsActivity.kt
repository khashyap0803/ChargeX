...
import com.chargex.india.fragment.MapFragmentArgs
import com.chargex.india.model.ChargeLocation
import com.chargex.india.navigation.NavHostFragment
import com.chargex.india.storage.PreferenceDataSource
import com.chargex.india.utils.getLocationFromIntent
...
class MapsActivity : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        } else if (intent.scheme == "com.chargex.india") {
            intent.data?.let {
    ...
    fun showLocation(charger: ChargeLocation, rootView: View) {
        ...
        if (pkg == null) {
            // There is no default maps app or ChargeX itself is the current default, fall back to app chooser
    ...
    fun openUrl(url: String, rootView: View, preferBrowser: Boolean = false) {
        ...
        if (preferBrowser) {
            // ChargeX may be set as default app for this link, but we want to open it in a browser
            ...
            if (pkg == null) {
                ...
                // add missing browsers (if ChargeX is already set as default, Android might not find other browsers with the specific intent)
