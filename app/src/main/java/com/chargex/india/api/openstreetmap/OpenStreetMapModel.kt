...
import com.chargex.india.model.Address
...
import com.chargex.india.model.OpeningHours
...
data class OSMChargingStation(
    ...
    private fun getChargepoints(): List<Chargepoint> {
        ...
        for (socket in SOCKET_TYPES) {
            ...
            if (count > 0) {
                if (socket.chargexKey != null) {
                    val outputPower = parseOutputPower(this.tags["${socket.osmSocketBaseTag()}:output"])
                    chargepoints.add(Chargepoint(socket.chargexKey, outputPower, count))
                }
            }
        }
        return chargepoints
