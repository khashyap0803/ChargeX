...
import com.chargex.india.EXTRA_DONATE
import com.chargex.india.MapsActivity
import com.chargex.india.R
import com.chargex.india.addDebugInterceptors
import com.chargex.india.api.availability.tesla.TeslaAuthenticationApi
import com.chargex.india.api.availability.tesla.TeslaOwnerApi
import com.chargex.india.fragment.oauth.OAuthLoginFragmentArgs
import com.chargex.india.getPackageInfoCompat
...
@ExperimentalCarApi
class SettingsScreen(ctx: CarContext, val session: ChargeXSession) : Screen(ctx), DefaultLifecycleObserver {
...
@ExperimentalCarApi
class DataSettingsScreen(ctx: CarContext, val session: ChargeXSession) : Screen(ctx), DefaultLifecycleObserver {
    val prefs = PreferenceDataSource(ctx)
...
@ExperimentalCarApi
class AboutScreen(ctx: CarContext, val session: ChargeXSession) : Screen(ctx) {
    val prefs = PreferenceDataSource(ctx)
...
@ExperimentalCarApi
class AcceptPrivacyScreen(ctx: CarContext, val session: ChargeXSession) : Screen(ctx) {
    val prefs = PreferenceDataSource(ctx)
    override fun onGetTemplate(): Template {
