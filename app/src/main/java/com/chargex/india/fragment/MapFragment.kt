...
import com.chargex.india.adapter.DetailsAdapter
import com.chargex.india.adapter.GalleryAdapter
import com.chargex.india.adapter.PlaceAutocompleteAdapter
import com.chargex.india.autocomplete.ApiUnavailableException
import com.chargex.india.autocomplete.PlaceWithBounds
import com.chargex.india.bold
import com.chargex.india.databinding.FragmentMapBinding
import com.chargex.india.location.FusionEngine
import com.chargex.india.location.LocationEngine
import com.chargex.india.location.Priority
import com.chargex.india.model.ChargeLocation
import com.chargex.india.model.ChargepointListItem
import com.chargex.india.model.ChargerPhoto
import com.chargex.india.model.FILTERS_CUSTOM
import com.chargex.india.model.FILTERS_DISABLED
import com.chargex.india.model.FILTERS_FAVORITES
import com.chargex.india.model.WaitTimeEngine
import com.chargex.india.model.StationScorer
import com.chargex.india.model.StationData
import com.chargex.india.navigation.safeNavigate
import com.chargex.india.shouldUseImperialUnits
import com.chargex.india.storage.PreferenceDataSource
import com.chargex.india.ui.HideOnScrollFabBehavior
import com.chargex.india.ui.MarkerManager
import com.chargex.india.ui.setTouchModal
import com.chargex.india.utils.boundingBox
import com.chargex.india.utils.checkAnyLocationPermission
import com.chargex.india.utils.checkFineLocationPermission
import com.chargex.india.utils.distanceBetween
import com.chargex.india.utils.formatDecimal
import com.chargex.india.viewmodel.GalleryViewModel
import com.chargex.india.viewmodel.MapPosition
import com.chargex.india.viewmodel.MapViewModel
import com.chargex.india.viewmodel.Resource
...
class MapFragment : Fragment(), OnMapReadyCallback, MenuProvider {
    ...
    private var pendingReachableBoundary: List<Pair<Double, Double>>? = null
        ...
        findNavController().currentBackStackEntry?.savedStateHandle
            ...
            ?.observe(viewLifecycleOwner) { rangeKm ->
                if (rangeKm > 0) {
                    ...
                    Snackbar.make(
                        binding.root,
                        "Calculating reachable area for %.0f km...".format(rangeKm),
                    ...
                    markerManager?.let { mm ->
                        ...
                        mm.reachableBoundary = null // reset old polygon before fetching new
                        mm.rangeFilterKm = rangeKm
                        vm.chargepoints.value?.data?.let { cp ->
                            android.util.Log.d("MapFragment", "Immediate re-filter (fallback): ${cp.size} chargepoints")
                            mm.chargepoints = cp
                        }
                    }

                    // Fetch reachable polygon in background
                    if (userLatLng != null) {
                        lifecycleScope.launch {
                            val boundary = com.chargex.india.api.RouteService.getReachableRange(
                                userLatLng!!.latitude, userLatLng!!.longitude,
                                (rangeKm * 1000).toInt()
                            )
                            pendingReachableBoundary = boundary
                            markerManager?.let { mm ->
                                if (boundary != null) {
                                    android.util.Log.d("MapFragment", "Applying new reachable polygon to MarkerManager")
                                    mm.reachableBoundary = boundary
                                    vm.chargepoints.value?.data?.let { cp -> mm.chargepoints = cp }
                                    Snackbar.make(
                                        binding.root,
                                        "Range filter applied (actual road distance)",
                                        Snackbar.LENGTH_SHORT
                                    ).show()
                                } else {
                                    Snackbar.make(
                                        binding.root,
                                        "Range filter applied (estimated road distance)",
                                        Snackbar.LENGTH_SHORT
                                    ).show()
                                }
                            }
    ...
    private fun setupClickListeners() {
        ...
        binding.fabOfflineVerify.setOnClickListener {
            findNavController().navigate(R.id.action_map_to_booking_verification)
        }
        binding.fabWallet.setOnClickListener {
            findNavController().navigate(R.id.action_map_to_wallet)
        }
        binding.fabDownloadStations.setOnClickListener {
            vm.mapPosition.value?.bounds?.let { bounds ->
                vm.downloadOfflineRegion(bounds)
                Snackbar.make(binding.root, "📥 Downloading and caching all stations in this area...", Snackbar.LENGTH_LONG).show()
            } ?: Snackbar.make(binding.root, "Pan the map first, then tap Download", Snackbar.LENGTH_SHORT).show()
        }
        binding.layers.btnClose.setOnClickListener {
            closeLayersMenu()
        }
        binding.layers.btnSyncOffline.setOnClickListener {
            vm.mapPosition.value?.bounds?.let { bounds ->
                vm.downloadOfflineRegion(bounds)
                Snackbar.make(binding.root, "Downloading and caching stations in this area...", Snackbar.LENGTH_LONG).show()
                closeLayersMenu()
            }
        }
        ...
        binding.detailView.btnBookStation.setOnClickListener {
            findNavController().navigate(R.id.action_map_to_booking_verification)
        }
    ...
    override fun onMapReady(map: AnyMap) {
        ...
        markerManager = MarkerManager(context, map, this).apply {
            ...
            if (pendingRangeFilterKm > 0f) {
                ...
                reachableBoundary = pendingReachableBoundary
    ...
    private fun displayWaitTimePrediction(charger: ChargeLocation) {
        ...
        val isOffline = !com.chargex.india.isNetworkAvailable(requireContext())
        ...
        // Show wait time as toolbar subtitle with offline warning icon if disconnected
        val subtitle = if (prediction.isAvailable) {
            if (isOffline) "\u26A0\uFE0F ${prediction.statusMessage} \u2022 ${prediction.queueProbabilityPercent} queue chance"
            else "\u26A1 ${prediction.statusMessage} \u2022 ${prediction.queueProbabilityPercent} queue chance"
        } else {
            if (isOffline) "\u26A0\uFE0F ${prediction.statusMessage} \u2022 ${prediction.areaProfile.displayName}"
            else "\u231B ${prediction.statusMessage} \u2022 ${prediction.areaProfile.displayName}"
        }
    ...
    override fun onDestroyView() {
        // Release marker references without calling marker.remove() on the
        // already-destroyed NativeMapView (avoids 200+ removeAnnotation errors)
        markerManager?.cleanupWithoutRemove()
