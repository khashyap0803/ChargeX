...
private const val backupFileName = "chargex-backup.db"
...
class BackupAgent : BackupAgent() {
    ...
    override fun onFullBackup(data: FullBackupDataOutput) {
        runBlocking {
            // creates a backup of the app database to chargex-backup.db
    ...
    override fun onRestoreFinished() {
        ...
        // rename restored backup DB as chargex.db
        val backupDb = applicationContext.getDatabasePath(backupFileName)
        if (backupDb.exists()) {
            backupDb.renameTo(applicationContext.getDatabasePath("chargex.db"))
        }
        // clear cache age
