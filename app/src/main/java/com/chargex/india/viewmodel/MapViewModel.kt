...
import com.chargex.india.api.availability.AvailabilityRepository
import com.chargex.india.api.availability.ChargeLocationStatus
import com.chargex.india.api.availability.tesla.Pricing
import com.chargex.india.api.createApi
import com.chargex.india.api.openchargemap.OCMConnection
import com.chargex.india.api.openchargemap.OCMReferenceData
import com.chargex.india.api.stringProvider
...
import com.chargex.india.model.Favorite
...
import com.chargex.india.model.getMultipleChoiceValue
import com.chargex.india.model.getSliderValue
import com.chargex.india.storage.AppDatabase
import com.chargex.india.storage.ChargeLocationsRepository
import com.chargex.india.storage.EncryptedPreferenceDataStore
import com.chargex.india.storage.FilterProfile
import com.chargex.india.storage.PreferenceDataSource
import com.chargex.india.ui.cluster
import com.chargex.india.utils.distanceBetween
import com.chargex.india.utils.normalize
...
class MapViewModel(application: Application, private val state: SavedStateHandle) :
    ...
    private var currentDetailSource: androidx.lifecycle.LiveData<com.chargex.india.viewmodel.Resource<com.chargex.india.model.ChargeLocation>>? = null
    private val detailObserver = androidx.lifecycle.Observer<com.chargex.india.viewmodel.Resource<com.chargex.india.model.ChargeLocation>> { response ->
        if (response.status == com.chargex.india.viewmodel.Status.SUCCESS) {
    ...
    fun downloadOfflineRegion(bounds: LatLngBounds) {
        val filters = filtersWithValue.value
        val source = repo.getChargepoints(bounds, zoom = 14f, filters, overrideCache = true)
        val observer = object : androidx.lifecycle.Observer<Resource<List<ChargepointListItem>>> {
            override fun onChanged(value: Resource<List<ChargepointListItem>>) {
                if (value.status == Status.SUCCESS || value.status == Status.ERROR) {
                    ...
                    if (value.status == Status.SUCCESS) {
                         val stations = value.data?.filterIsInstance<ChargeLocation>() ?: emptyList()
                         android.util.Log.d("OfflineSync", "Syncing ${stations.size} stations for offline")
                         stations.forEach { station ->
                             val detailSource = repo.getChargepointDetail(station.id, overrideCache = true)
                             val detailObserver = object : androidx.lifecycle.Observer<Resource<ChargeLocation>> {
                                 override fun onChanged(detailValue: Resource<ChargeLocation>) {
                                     if (detailValue.status == Status.SUCCESS || detailValue.status == Status.ERROR) {
                                         detailSource.removeObserver(this)
                                     }
                                 }
                             }
                             detailSource.observeForever(detailObserver)
                         }
                    }
                }
            }
